...
def int8_cartesian(vecs_f32: np.ndarray) -> np.ndarray:
    """
    Apply standard int8 per-dim quantization to Cartesian unit vectors and
    return the (dequantized, renormalized) float32 form used for cosine.

    Scale chosen as 127 because text-embedding-3-small components live in
    roughly [-1, 1] (actually tighter, but the full range is safe).
    Storage cost: 8 bits per dimension = 8 * EMBED_DIM bits per vector.
    """
    scale = 127.0
    q = np.round(np.clip(vecs_f32, -1.0, 1.0) * scale).astype(np.int8)
...
def residual_int8(original_f32: np.ndarray,
                  stage1_recon_f32: np.ndarray,
                  residual_bits: int = 8) -> tuple[np.ndarray, float]:
    """
    Residual quantization: int8-quantize (original - stage1_recon).

    Because the residual has much smaller dynamic range than the original
    vectors (it's the ERROR of stage 1, not the embedding itself), the int8
    bins are effectively finer on the residual than they would be on the
    raw vector. This is why residual stacks add real fidelity rather than
    just re-quantizing the same scale.

    Returns:
        reconstructed: stage1_recon + dequant(quant(residual))
        scale_used: the per-axis scale factor, needed to dequantize at read time

    The int8 codes themselves are what you'd store in practice; here we
    return the reconstruction for the retrieval comparison.
    """
    residual = original_f32 - stage1_recon_f32
    # Use a global scale based on the actual residual magnitude, not [-1,1].
    # This is where the fidelity gain comes from.
    r_max = float(np.abs(residual).max())
    if r_max < 1e-12:
        return stage1_recon_f32.copy(), 1.0
    levels = (1 << (residual_bits - 1)) - 1   # 127 for int8
    scale = r_max / levels
    codes = np.round(residual / scale).astype(np.int32)
    codes = np.clip(codes, -levels, levels).astype(np.int8)
    r_hat = codes.astype(np.float32) * scale
    return stage1_recon_f32 + r_hat, scale
...
def main():
    ...
    for budget in BIT_BUDGETS:
        ...
        # Residual-stacked: store dyn-tier codes AND an int8 quantization of
        # the residual (original - stage1 reconstruction). Total storage is
        # the sum of both stages. This is the correct "sequential" form:
        # stage 2 captures error stage 1 missed, total fidelity strictly
        # improves over stage 1 alone.
        #
        # Both `doc_vecs` and `doc_recon` are unit vectors, so the residual
        # is their direct difference. Its magnitude is bounded by 2 but
        # typically much smaller (0.01 - 0.5 for reasonable budgets), which
        # is why int8 on the residual has finer effective resolution than
        # int8 on the raw embedding.
        doc_stacked, _  = residual_int8(doc_vecs, doc_recon)
        q_stacked, _    = residual_int8(q_vecs,   q_recon)
        doc_stacked_n = doc_stacked / np.clip(np.linalg.norm(doc_stacked, axis=1, keepdims=True), 1e-12, None)
        q_stacked_n   = q_stacked   / np.clip(np.linalg.norm(q_stacked,   axis=1, keepdims=True), 1e-12, None)

        stacked_bits = actual_bits + 8 * EMBED_DIM   # both stages stored
        stacked = evaluate(
            label=f"dyn-tier {budget:>5}b +res-int8",
            bits_per_vec=stacked_bits,
            query_unit=q_stacked_n, corpus_unit=doc_stacked_n,
            truth=truth, k=TOP_K,
            tier_breakdown=tier_summary(tiers),
        )
        results.append(stacked)

        # Asymmetric version of the residual stack: float32 query, two-stage docs.
        stacked_asym = evaluate(
            label=f"dyn-tier {budget:>5}b +res-int8 ASYM(q=f32)",
            bits_per_vec=stacked_bits,
            query_unit=q_vecs, corpus_unit=doc_stacked_n,
            truth=truth, k=TOP_K,
            tier_breakdown=tier_summary(tiers),
        )
        results.append(stacked_asym)
